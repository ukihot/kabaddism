...
//!
//! `app`（bevy）層はフェーズ7で載せる。それまでは、このバイナリで1年を回して
//! バランスを確かめる。`cargo run -- --seed 42 --years 1`

use std::sync::Arc;

use kbism::sim::calendar::StopReason;
use kbism::sim::defs::{BudgetField, Defs, TargetScope};
...
/// 自動プレイ用の予算配分。収入見込みを8分野へ配る。
fn auto_budget(brief: &kbism::sim::budget::BudgetBriefing) -> [f32; 8] {
    let pot = brief.allocatable().max(0.0);
    let weights = [0.16, 0.13, 0.15, 0.13, 0.17, 0.12, 0.09, 0.05];
    let mut out = [0.0f32; 8];
    for (i, w) in weights.iter().enumerate() {
        out[i] = pot * w;
    }
    let _ = BudgetField::ALL;
    let _ = TargetScope::District;
