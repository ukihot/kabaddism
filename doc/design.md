...
> 現状: 上表は `tests/sim_integration.rs` に揃っている（決定論・セーブ往復・
> 会計恒等式・政策回帰・完走・性能）。政策回帰は3シードのうち1つでも期待方向が
> 出れば良しとする（イベントの確率で単一シードの結果が偶然崩れることがあるため）。
> `athlete_support` は対象が1地区・効果が indirect（obligation 経由）で、国全体
> 平均の `PracticeMinutes` には信号が薄すぎるため対象から外している。地区別の
> practice 指標を `stats::DistrictStats` に足すのがフェーズ9の宿題。
